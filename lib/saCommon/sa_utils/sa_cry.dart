...
class SACryptoUtil {
  ...
  static String encryptUrl({required String originalUrl, String prefix = 'alpha'}) {
    ...
    // 2. 加密整个路径（包括查询参数）
    final encryptedPathContent = originalPathWithQuery.isNotEmpty ? encrypt(originalPathWithQuery) : '';
    _log('加密后路径: $encryptedPathContent');

    // 3. 构建新 URL (域名 + /alpha/加密内容)
    final newUri = Uri(scheme: uri.scheme, host: uri.host, port: uri.port, path: '/$prefix/$encryptedPathContent');
