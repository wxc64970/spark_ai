...
class SAPayUtils {
  // 单例模式
  static final SAPayUtils _instance = SAPayUtils._internal();
  factory SAPayUtils() => _instance;
  SAPayUtils._internal() {
    _initIAP();
  }
  ...
  List<SASkModel> allList = [];
  List<SASkModel> consumableList = [];
  List<SASkModel> subscriptionList = [];

  VipFrom? _vipFrom;
  ConsumeFrom? _consFrom;
  SAOrderModel? _orderData;

  bool _isUserBuy = false;
  SASkModel? _currentSkuData;
  ...
  Future<void> buy(SASkModel data, {VipFrom? vipFrom, ConsumeFrom? consFrom}) async {
    try {
      ...
      if (productDetails == null) {
        ...
        SAToast.show('So sorry, ProductDetails is null.');
        return;
      }
      ...
      if (orderNo == null || orderNo.isEmpty) {
        SALoading.close();
        SAToast.show(SATextData.createOrderError);
        return;
      }
  ...
  Future<void> _createOrder(ProductDetails productDetails) async {
    ...
    if (Platform.isIOS) {
      try {
        ...
        // SAToast.show('${L10nHelper.current.createOrderError} $e');
        SAToast.show('So sorry, create order error. $e');
        rethrow;
      }
    }
  ...
  Future<bool> _isAvailable() async {
    ...
    if (!isAvailable) {
      await SALoading.show();
      // SAToast.show(L10nHelper.current.iapNotSupport);
      SAToast.show('So sorry, IAP not support.');
    }
    return isAvailable;
  }
