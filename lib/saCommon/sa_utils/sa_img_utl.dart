...
class SAImageUtils {
  ...
  static Future<File?> pickImageFromGallery({double? maxWidth, double? maxHeight, int imageQuality = 85}) async {
    try {
      final XFile? pickedFile = await _picker.pickImage(source: ImageSource.gallery, maxWidth: maxWidth, maxHeight: maxHeight, imageQuality: imageQuality);
  ...
  static Future<File?> pickImageFromCamera({double? maxWidth, double? maxHeight, int imageQuality = 85}) async {
    try {
      final XFile? pickedFile = await _picker.pickImage(source: ImageSource.camera, maxWidth: maxWidth, maxHeight: maxHeight, imageQuality: imageQuality);
  ...
  static Future<List<File>> pickMultipleImages({double? maxWidth, double? maxHeight, int imageQuality = 85, int? limit}) async {
    try {
      final List<XFile> pickedFiles = await _picker.pickMultiImage(maxWidth: maxWidth, maxHeight: maxHeight, imageQuality: imageQuality, limit: limit);

      if (pickedFiles.isNotEmpty) {
        debugPrint('ImageUtils.pickMultipleImages ✅: ${pickedFiles.length} images selected');
  ...
  static Future<File?> cropImage(File file, {int? aspectRatioX, int? aspectRatioY, int? maxWidth, int? maxHeight, int compressQuality = 90}) async {
    try {
      final CroppedFile? croppedFile = await ImageCropper().cropImage(
        sourcePath: file.path,
        aspectRatio: (aspectRatioX != null && aspectRatioY != null) ? CropAspectRatio(ratioX: aspectRatioX.toDouble(), ratioY: aspectRatioY.toDouble()) : null,
        maxWidth: maxWidth,
        maxHeight: maxHeight,
        ...
        uiSettings: [
          ...
          IOSUiSettings(title: '裁剪图片', aspectRatioLockEnabled: aspectRatioX != null && aspectRatioY != null, resetAspectRatioEnabled: true, aspectRatioPickerButtonHidden: false),
        ],
      );
  ...
  /// [compressQuality] 压缩质量（0-100）
  /// 返回裁剪后的图片文件，取消或失败时返回null
  static Future<File?> cropImageSquare(File file, {int? size, int compressQuality = 90}) async {
    return cropImage(file, aspectRatioX: 1, aspectRatioY: 1, maxWidth: size, maxHeight: size, compressQuality: compressQuality);
  }
  ...
  /// 返回压缩后的文件，失败时返回null
  static Future<File?> compressAndConvertToJpg(File file, {int initialQuality = 85, int minSize = 2 * 1024 * 1024}) async {
    try {
  ...
  /// [extension] 文件扩展名
  /// 返回带时间戳的唯一文件名
  static String generateUniqueFileName({String prefix = 'image', String extension = 'jpg'}) {
  ...
  /// [minSize] 目标文件大小
  /// 返回处理后的文件列表
  static Future<List<File>> batchProcessImages(List<File> files, {int initialQuality = 85, int minSize = 2 * 1024 * 1024}) async {
    final List<File> processedFiles = [];

    for (final file in files) {
      final processed = await compressAndConvertToJpg(file, initialQuality: initialQuality, minSize: minSize);
      if (processed != null) {
        processedFiles.add(processed);
      }
    }

    debugPrint('ImageUtils.batchProcessImages ✅: ${processedFiles.length}/${files.length} processed');
  ...
  static Future<int> batchDeleteImageFiles(List<File> files) async {
    ...
    debugPrint('ImageUtils.batchDeleteImageFiles ✅: $deletedCount/${files.length} deleted');
    return deletedCount;
  }
}
