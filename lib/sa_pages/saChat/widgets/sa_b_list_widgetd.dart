...
abstract class BaseListView<T, C extends SABaseListController<T>> extends StatefulWidget {
  const BaseListView({super.key, this.emptyBuilder, this.enablePullRefresh = true, this.enableLoadMore = true, this.padding, this.cacheExtent, required this.controller});
  ...
  final SABaseListController<T> controller;
...
class _BaseListViewState<T, C extends SABaseListController<T>> extends State<BaseListView<T, C>> {
  @override
  void initState() {
    super.initState();
      ...
      childBuilder: (context, physics) {
        return Obx(() {
          if (widget.controller.emptyType.value != null || widget.controller.dataList.isEmpty) {
            return widget.emptyBuilder?.call(context) ?? EmptyWidget(type: widget.controller.emptyType.value!, physics: physics);
          }
