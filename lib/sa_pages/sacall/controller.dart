...
class SacallController extends GetxController {
  SacallController();

  final state = SacallState();
  ...
  var answerText = '';
  SAMsgReplayModel? messageReplyRsp;
  ...
  bool get _isVip => SA.login.vipStatus.value;
  ...
  void _getArgs() {
    ...
    log.d('_getArgs _showVideo: $_showVideo, _hasVideoPlayer: $_hasVideoPlayer');
  ...
  String callStateDescription(CallState callState) {
    switch (callState) {
      ...
      case CallState.listening:
        return SATextData.listening;
      case CallState.answering:
        return SATextData.waitForResponse;
  ...
  void onTapAccept() {
    _stopVibration();
    if (!_isVip) {
      SAlogEvent('acceptcall');
      Get.toNamed(SARouteNames.vip, arguments: VipFrom.acceptcall);
      return;
    }
    onTapCall();
  }
  ...
  void _showPermissionDialog() {
    DialogWidget.alert(
      message: SATextData.microphonePermissionRequired,
  ...
  void _onCallTimeout() {
    log.d('_onCallTimeout, callState: ${callState.value}, currentRoute: ${RoutePages.observer.curRoute?.settings.name}');
    _stopVibration();
    ...
    if (callState.value == CallState.incoming) {
      ...
      log.d('_onCallTimeout - current route: $currentRouteName, expected: ${SARouteNames.phone}');

      if (currentRouteName == SARouteNames.phone || currentRouteName == null) {
  ...
  Future<void> _deductGems() async {
    if (SA.login.checkBalance(ConsumeFrom.call)) {
      SA.login.deductBalance(ConsumeFrom.call);
    } else {
      SAToast.show(SATextData.notEnoughCoins);
  ...
  Future<void> _initSpeech() async {
    try {
      ...
      _hasSpeech = await _speech.initialize(
        ...
        onError: (error) {
          log.e('onError: $error');
          if (error.errorMsg.contains('error_language_not_supported') || error.errorMsg.contains('error_language_unavailable')) {
            Get.back();
            DialogWidget.alert(
              title: SATextData.tips,
              message: SATextData.speechRecognitionNotSupported,
              onConfirm: () {
      ...
      SAToast.show(SATextData.speechRecognitionNotSupported);
    }
  }
  ...
  Future<SAMsgReplayModel?> _sendMessage() async {
    log.d('_sendMessage: ${lastWords.value}');
    final roleId = role.id;
    var userId = SA.login.currentUser?.id;
    var nickname = SA.login.currentUser?.nickname;
    if (roleId == null || userId == null || nickname == null) {
      SAToast.show(SATextData.someErrorTryAgain);
      return null;
    }

    final res = await Api.sendVoiceChatMsg(userId: userId, nickName: nickname, message: lastWords.value, roleId: roleId);
  ...
  void _playResponseAudio(SAMsgReplayModel msg) async {
  ...
  void _listenToAudioState(String msgId) {
    // 方法1: 监听全局播放状态变化
    ever(SA.audio.currentPlayingAudio, (audioInfo) {
